...
export function Room(props: React.ComponentProps<"group">) {
  ...
  // _luma palette — cool, electric, dark tech
  const curtainMaterial = new THREE.MeshPhongMaterial({ color: "#0d1b4b" });      // deep navy
  const bodyMaterial = new THREE.MeshPhongMaterial({ map: matcapTexture });
  const tableMaterial = new THREE.MeshPhongMaterial({ color: "#0a0a14" });         // almost black
  const radiatorMaterial = new THREE.MeshPhongMaterial({ color: "#c8d6e5" });      // cool grey
  const compMaterial = new THREE.MeshStandardMaterial({ color: "#e8eaf6" });       // light grey-blue
  const pillowMaterial = new THREE.MeshPhongMaterial({ color: "#4cc9f0" });        // electric cyan
  const chairMaterial = new THREE.MeshPhongMaterial({ color: "#0d0d1a" });         // near-black navy
